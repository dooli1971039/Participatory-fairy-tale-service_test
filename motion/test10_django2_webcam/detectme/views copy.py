...
class Openpose(object):
    ...
    def get_frame(self,request,pose_type):
        global status,return_result
        self.status=status
...
def gen(request,camera,pose_type):
    ...
    while True:
        frame = camera.get_frame(request,pose_type)
    ...
    return "END"        
    # return_render=changeResult(request)
    # print("#111")
...
def detectme_Stretching(request):
    try:
        streaming=gen(request,Openpose(),"Stretching")
...
def changeResult(request):
    ...
    context={}
    if return_result!="":
        context={"return_result":return_result}
